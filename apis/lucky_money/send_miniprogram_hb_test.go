...
func TestSendMiniprogramHB(t *testing.T) {
	test.Setup()
	...
	var resp []byte
	test.MockSvrHandler.HandleFunc("/mmpaymkttransfers/sendminiprogramhb", func(w http.ResponseWriter, r *http.Request) {
		w.Write([]byte(resp))
	})
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			gotResult, err := SendMiniprogramHB(test.MockWXPay, tt.args.params)
			if (err != nil) != tt.wantErr {
				t.Errorf("SendMiniprogramHB() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
