...
func TestMicroPay(t *testing.T) {
	...
	var resp []byte
	test.MockSvrHandler.HandleFunc("/pay/micropay", func(w http.ResponseWriter, r *http.Request) {
		w.Write([]byte(resp))
	})
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			gotResult, err := MicroPay(test.MockWXPay, tt.args.params)
			if (err != nil) != tt.wantErr {
				t.Errorf("MicroPay() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
