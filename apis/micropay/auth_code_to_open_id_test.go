...
func TestAuthCodeToOpenId(t *testing.T) {
	...
	var resp []byte
	test.MockSvrHandler.HandleFunc("/tools/authcodetoopenid", func(w http.ResponseWriter, r *http.Request) {
		w.Write([]byte(resp))
	})
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			gotResult, err := AuthCodeToOpenId(test.MockWXPay, tt.args.params)
			if (err != nil) != tt.wantErr {
				t.Errorf("AuthCodeToOpenId() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
