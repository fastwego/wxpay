...
func TestProfitSharingAddReceiver(t *testing.T) {
	test.Setup()
	...
	var resp []byte
	test.MockSvrHandler.HandleFunc("/pay/profitsharingaddreceiver", func(w http.ResponseWriter, r *http.Request) {
		w.Write([]byte(resp))
	})
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			gotResult, err := ProfitSharingAddReceiver(test.MockWXPay, tt.args.params)
			if (err != nil) != tt.wantErr {
				t.Errorf("ProfitSharingAddReceiver() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
